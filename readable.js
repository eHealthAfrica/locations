...
var decorateCoordinates = function (coordinates) {
  var res = ''
  if (coordinates) {
    res += ' [' + coordinates.lon + ',' + coordinates.lat + ']'
  }
  return res
}
...
var getSubRegions = function (index, parentId, indentation, isLast) {
  if (data[index]) {
    ...
    subregions.forEach(function (region, listNumber) {
      ...
      console.log(thisIndentation + toTitleCase(region.name) + ' (' + region.id + ')' + decorateCoordinates(region.coordinates))
      var subIndentation
      if (isLast) {
