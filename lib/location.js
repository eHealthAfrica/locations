...
Location.loadNested = function (data, parent) {
  ...
  _.forEach(data, function (value, indexKey, collection) {
    var location = new Location(_.isArray(collection) ? value : indexKey, parent)
...
Location.loadItems = function (items, root) {
  ...
  _.forEach(items, function (item, indexKey, collection) {
...
Location.loadFlatten = function (data, root) {
  ...
  _.forEach(data, function (level, indexKey, collection) { Location.loadItems(level.items, root) })
  return root
}
...
Location.sortById = function (collection) {
  return _.sortBy(collection, function (element) {
    var withoutDashes = element.id.replace(/-/g, '')
    var res = 0
    // check if it can be coerced to number
    // (+ is number coercion); different to parseInt
    if (+withoutDashes) {
      res = +withoutDashes
    }
    return res
...
Location.prototype = {
  ...
  toJSON: function () {
    ...
    if (this.level() === 0) {
      ...
      return Location.toDataModel(this, _.map(
        new Array(this.depth() + 1), function (value, indexKey) {
          return { depth: undefined, name: names[indexKey], items: [] }
        }))
    } else {
      this.toJSON2()
    }
   ...
   */
  toJSON2: function () {
    var res = { name: this.name, id: this.getId() }
    if (this.ochaId) {
